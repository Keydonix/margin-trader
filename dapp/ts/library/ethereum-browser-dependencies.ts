...
import { Dependencies, TransactionReceipt as PartialTransactionReceipt } from '../generated/margin-trader'
import { ReadonlyFetchJsonRpcDependencies } from './readonly-fetch-dependencies';
import { unknownErrorToJsonRpcError } from './error-handler';

declare global {
	interface Ethereum1193 {
		request(options: { readonly method: JsonRpcMethod, readonly params: unknown[] }): Promise<unknown>
	}
	interface EthereumLegacy {
		sendAsync(options: { jsonrpc: '2.0', id: number | string | null, method: JsonRpcMethod, params: readonly unknown[] }, callback: (error: Error, response: unknown) => void): void
	}
...
export class EthereumBrowserDependencies implements Dependencies {
	...
	public readonly enable = async (): Promise<readonly bigint[]> => {
		const addressStrings = (window.ethereum && 'enable' in window.ethereum)
			? await window.ethereum.enable()
			: await this.send('eth_accounts') as readonly string[]
		if (addressStrings.length !== 0) this.enabled = true
		const accounts = addressStrings.map(addressString => BigInt(addressString))
		if (accounts.length > 0) this.address = accounts[0]
		return accounts
	}

	public readonly noBrowserExtensionNeeded = () => 'ethereum' in window

	private readonly send = async (method: JsonRpcMethod, ...params: unknown[]): Promise<unknown> => {
		if (window.ethereum === undefined) throw new Error(`Your browser does not appear to be web3 enabled.`)
		if ('request' in window.ethereum) {
			...
			return new Promise((resolve, reject) => {
				ethereum.sendAsync({ jsonrpc: '2.0', id: 0, method, params }, (error, result) => {
					...
					if (!isJsonRpcLike(result)) return reject(new Error(`Your browser appears to be a web3 browser, but isn't compatible with this app.`))
			...
			throw new Error(`Your browser does not appear to be web3 enabled.`)
		}
	}
